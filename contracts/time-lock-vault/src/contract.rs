...
impl TimeLockVault {
    pub fn initialize(env: Env, admin: Address, fee_recipient: Address) -> Result<(), VaultError> {
        ...
        storage::set_max_deposit(&env, MAX_DEPOSIT_AMOUNT);
        storage::set_max_lock_secs(&env, MAX_LOCK_DURATION_SECS);
    ...
    pub fn cancel_deposit(env: Env, depositor: Address, deposit_id: u32) -> Result<(), VaultError> {
        ...
        let entry = storage::get_deposit(&env, &depositor, deposit_id).ok_or(VaultError::NoDepositFound)?;
        let now = env.ledger().timestamp();
        ...
        let penalty = (entry.amount * entry.penalty_bps as i128) / 10_000;
